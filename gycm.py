...
#
# Original file available at https://github.com/Valloric/ycmd/blob/master/examples/example_client.py
...
# The main class
class GYCM(geany.Plugin)
    __plugin_name = "YCM for Geany"
    __plugin_description__ = "Utilises YouCompleteMe's ycmd to provide smart code completion."
    __plugin_version__ = "0.01"
    __plugin_author__ = "Jake Bott - http://github.com/jakeanq"
    ...
    def cleanup(self):
        self.onSaveConfigTimeout()
        self.server.Shutdown()
    
    def loadCfg(self):
        self.cfgPath = os.path.join(geany.app.configdir, "plugins", "gycm.conf")
        self.cfg = SafeConfigParser()
        self.cfg.read(self.cfgPath)
    
    def saveConfig(self):
        gobject.idle_add(self.onSaveConfigTimeout)
    
    def onSaveConfigTimeout(self, data=None):
        self.cfg.write(open(self.cfgPath, 'w'))
        return False
    
    def onDocumentOpen(self, document,data=None):

        data = {
            'line_num': line_num,
            'column_num': column_num,
            'filepath': document.real_path,
            'file_data': {
                document.real_path: {
                    'filetypes': [ document.filetype.name ], # XXX may not work...
                    'contents': document.editor.sci.getText(
                }
            }
        }
